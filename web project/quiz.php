...
$sql = "
  SELECT course_titles.id
  FROM course_titles course_titles
  JOIN courses course ON course_titles.course_id = course.id
  WHERE course.name = ? AND course_titles.title = ?
  LIMIT 1
";
...
if ($row = $result->fetch_assoc()) {
    ...
    $quizStmt = $conn->prepare("SELECT * FROM quiz_questions WHERE topic_id = ?");
    ...
    while ($quiz = $quizResult->fetch_assoc()) {
        ...
        echo "<p>Q{$qnum}: " . htmlspecialchars($quiz['question_text']) . "</p>";
